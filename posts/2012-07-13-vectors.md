...
Se você já jogou Angry Birds, ou Cut the Hope, já deve ter se impressionado com o quanto "realísticos" esses jogos se parecem. E não para por aí, esses são apenas os exemplos mais famosos, já que é cada vez mais comum vermos jogos que fazem uso de cálculos físicos com extrema precisão. O fato é que simular eventos físicos não é um assunto novo na computação, alias, muito pelo contrário, anos de estudo já foram aplicados nessa área, e por conta disso, hoje em dia é possível com apenas um _google_, encontrar diversas bibliotecas que nos permitem criar simulações físicas extremamente avançadas. E é claro que é impossível não se maravilhar enquanto se navega pelas dezenas de exemplos dos mais variados casos que essas bibliotecas exibem, e o melhor: além de terem suporte a uma gigantesca variedade de linguagens, grande parte delas estão disponíveis gratuitamente, e na maior parte das vezes, com o código fonte aberto. É _óbvio_ que ter tudo isso em nossas mões tão facilmente faz com que nossa imaginação comece a maquinar as ideias mais mirabolantes possíveis, já que aparentemnte todos os problemas envolvendo física estão resolvidos!
